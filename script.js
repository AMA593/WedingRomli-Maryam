...
function bukaUndangan() {
    ...
    // 2. Sembunyikan Cover dengan animasi slide ke atas
    const cover = document.getElementById('wedding-cover');
    cover.classList.add('cover-hidden');

    // 3. Aktifkan scroll (jika sebelumnya dimatikan)
    document.body.style.overflow = "auto";

    // 4. Inisialisasi AOS (Animasi muncul saat scroll)
    if (typeof AOS !== 'undefined') {
        AOS.init();
    }
}

// Opsional: Matikan scroll saat cover masih aktif
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const tamu = urlParams.get('to');
    if (tamu) {
        document.getElementById('guest-named').innerText = tamu;
    }
});